...
class ChromeValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        for key in ("bookmarks", "preferences", "extensions", "tabs", "history", "autofill", "passwords"):
    ...
    def test_every_required_category_must_be_registered(self):
        for category in ("history", "autofill", "passwords"):
            with self.subTest(category=category), self.assertRaises(RuntimeError):
                validate(self.about, {key: value for key, value in self.prefs.items()
                                      if key != category + "Registered"}, "alice@10kr.co")
