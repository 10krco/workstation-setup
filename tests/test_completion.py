...
class CompletionTest(unittest.TestCase):
    ...
    def finish(self):
        return complete("alice", self.user, self.system, self.active, self.root,
                        self.activate, self.rollback, state_owner=None)
    ...
    def test_untrusted_privileged_markers_are_rejected(self):
        managed = self.root / "managed-users/alice"
        managed.unlink()
        managed.symlink_to(self.root / "password-set/alice")
        with self.assertRaises(PermissionError):
            self.finish()
        managed.unlink()
        managed.touch()
        (self.root / "managed-users").chmod(0o777)
        with self.assertRaises(PermissionError):
            self.finish()
